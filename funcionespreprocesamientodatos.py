...
def procesar_record_individual (record, mean_global, std_global, exp=7, fs = 300, dis_min = 0.25, alt_min = None, corte_bajo = 0.5, corte_alto = 40, ord = 4):
  ...
  if señal_ecg.size == 0:                                         #Comprueba que la señal no sea nula
    ...
    segmentos = segmentacion_complejo_qrs (señal_filtrada, exponente=exp, frec_muestreo=fs, distancia_minima=dis_min, altura_minima=alt_min) # Segmentación de la señal en función del complejo QRS
...
try:
        senal = obtener_señal_ecg(record)
    except ValueError as e:
        print(f"Error extrayendo señal: {e}")
        return None  # O array vacío, según convenga

    if senal.size == 0:
        print("Señal vacía, se omite el procesamiento.")
        return None  # O array vacío, según convenga
